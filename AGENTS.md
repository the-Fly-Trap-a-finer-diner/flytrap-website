...
- **The deploy transpiles the JSX; the repo does not.** `pages.yml` runs
  `.github/scripts/precompile-jsx.mjs` against the staged `_site`, so visitors get
  plain `.js` and no Babel. **This is not a build step for you** — the repo keeps
  its `.jsx` files and its Babel `<script>` tag, `python3 -m http.server` works
  with no tooling, and nothing generated is ever committed. Keep editing `.jsx`.
